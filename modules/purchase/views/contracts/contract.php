        ...
        <div class="panel-body">
          ...
          <div class="row">
             <div class="col-md-12">
                ...
                <div class="col-md-6">
                  <?php $service_category = (isset($contract) ? $contract->service_category : '');
                  echo render_input('service_category','service_category',$service_category); ?>
                </div>

                <div class="col-md-6 form-group">
                  <label for="vendor"><?php echo _l('vendor'); ?></label>
                  <select name="vendor" id="vendor" class="ajax-search" onchange="vendor_change(this); return false;" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>" >
        
                     <?php
                            if(isset($contract)) {
                                $rel_data = get_relation_data('vendors', $contract->vendor);
                                $rel_val  = get_relation_values($rel_data, 'vendors');
                                echo '<option value="' . $rel_val['id'] . '" selected>' . $rel_val['name'] . '</option>';
                            }

                            if(isset($ven) && is_numeric($ven)){
                               $rel_data = get_relation_data('vendors', $ven);
                                $rel_val  = get_relation_values($rel_data, 'vendors');
                                echo '<option value="' . $rel_val['id'] . '" selected>' . $rel_val['name'] . '</option>';
                            }
                          ?>  
                ...
                <div class="col-md-6">
                    <?php $payment_amount = (isset($contract) ? $contract->payment_amount : 0);
                    echo render_input('payment_amount','payment_amount',$payment_amount,'number',array('step' => 'any')); ?> 
                </div>
                ...
                <div class="col-md-6">
                  <?php $contract_value = (isset($contract) ? $contract->contract_value : 0);
                  echo render_input('contract_value','contract_value',$contract_value,'number',array('step' => 'any')); ?>
