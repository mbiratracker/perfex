...
<table class="table dt-table border table-striped">
 <thead>
  ...
  <tbody>
    ...
    <tr>
        <td><?php echo pur_html_entity_decode($commodity_group_type['id']); ?></td>
        <td><?php echo pur_html_entity_decode($commodity_group_type['commodity_group_code']); ?></td>
        <td><?php echo pur_html_entity_decode($commodity_group_type['name']); ?></td>
        <td><?php echo pur_html_entity_decode($commodity_group_type['order']); ?></td>
        <td><?php if($commodity_group_type['display'] == 0){ echo _l('not_display'); }else{echo _l('display');} ?></td>
        <td><?php echo pur_html_entity_decode($commodity_group_type['note']); ?></td>

        <td>
            <?php if (has_permission('purchase_settings', '', 'edit') || is_admin()) { ?>
              <a href="#" onclick="edit_commodity_group_type(this,<?php echo pur_html_entity_decode($commodity_group_type['id']); ?>); return false;" data-commodity_group_code="<?php echo pur_html_entity_decode($commodity_group_type['commodity_group_code']); ?>" data-name="<?php echo pur_html_entity_decode($commodity_group_type['name']); ?>" data-order="<?php echo pur_html_entity_decode($commodity_group_type['order']); ?>" data-display="<?php echo pur_html_entity_decode($commodity_group_type['display']); ?>" data-note="<?php echo pur_html_entity_decode($commodity_group_type['note']); ?>" class="btn btn-default btn-icon"><i class="fa fa-pencil-square"></i>
            </a>
            <?php } ?>
