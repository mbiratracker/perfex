...
foreach ($rResult as $aRow) {
    ...
    $list_approve_status = $this->ci->purchase_model->get_list_approval_details($aRow['id'],'pur_quotation');

    if ( (has_permission('purchase_quotations', '', 'edit') || is_admin()) && $aRow[db_prefix() . 'pur_estimates.status'] != 2 && count($list_approve_status) == 0) {
    ...
    $amount = e(app_format_money($aRow[db_prefix() . 'pur_estimates.total'], $base_currency->name));
    ...
    $row[] = e(app_format_money($aRow[db_prefix() . 'pur_estimates.total_tax'], $base_currency->name));
