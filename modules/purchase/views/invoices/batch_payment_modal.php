                    ...
                    <div class="col-sm-4">
                        <div class="form-group select-placeholder">
                            <select id="batch-payment-filter" class="selectpicker" name="client_filter"
                                data-width="100%"
                                data-none-selected-text="<?php echo _l('batch_payment_filter_by_vendor') ?>">
                                <option value=""></option>
                                <?php foreach ($vendors as $vendor) { ?>
                                <option value="<?php echo $vendor->userid; ?>"><?php echo $vendor->company; ?>
                                </option>
                                <?php } ?>
                            </select>
                        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <tr class="batch_payment_item" data-vendorid="<?php echo $invoice->vendor; ?>"
                                        data-invoiceId="<?php echo $invoice->id ?>">
                                        <td>
                                            <a href="<?php echo admin_url('purchase/purchase_invoice/' . $invoice->id); ?>"
                                                target="_blank">
                                                <?php echo pur_html_entity_decode($invoice->invoice_number) ?>
                                            </a><br>
                                            <a class="text-dark"
                                        ...
                                        <td>
                                            <div class="form-group">
                                                <select class="selectpicker"
                                                    ...
                                                    data-none-selected-text="-" data-live-search="true">
                                                    <option></option>
                                                    <?php foreach ($payment_modes as $mode) { ?>
                                                    <option value="<?php echo pur_html_entity_decode($mode['id']); ?>"><?php echo pur_html_entity_decode($mode['name']); ?>
                                                    </option>
                                                    <?php } ?>
                                                </select>
                                            </div>
