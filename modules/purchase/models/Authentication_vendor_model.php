...
class Authentication_vendor_model extends App_Model
{
    ...
    public function login($email, $password, $remember, $staff)
    {
        if ((!empty($email)) and (!empty($password))) {
            ...
            if ($staff == true) {
                ...
                hooks()->do_action('before_vendor_login', [
                    'email'           => $email,
                    'userid'          => $user->userid,
                    'vendor_contact_user_id' => $user->$_id,
                ]);

                $user_data = [
                    'vendor_user_id'   => $user->userid,
                    'vendor_contact_user_id'  => $user->$_id,
    ...
    public function logout($staff = true)
    {
        ...
        if (is_vendor_logged_in()) {
            hooks()->do_action('before_contact_logout', get_vendor_user_id());
    ...
    public function forgot_password($email, $staff = false)
    {
        ...
        if ($user) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                if ($staff == false) {
                    $sent = send_mail_template('vendor_contact_forgot_password', 'purchase',$user->email, $user->userid, $user->$_id, $data);
                } else {
    ...
    public function reset_password($staff, $userid, $new_pass_key, $password)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $merge_fields = [];
            if ($staff == false) {
                $sent = send_mail_template('vendor_contact_password_resetted', 'purchase',$user->email, $user->userid, $user->$_id);
            } else {
