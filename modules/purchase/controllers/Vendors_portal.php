...
class Vendors_portal extends App_Controller
{
    ...
    */
    ...
    {
        if(get_option('purchase_active_vendor_portal') == 2){
            show_404();
        }
    ...
    public function add_update_quotation($id = '',$view = ''){
        ...
        if(get_option('allow_vendor_add_edit_delete_purchase_quotation') == 0){
            show_404();
        }
    ...
    public function invoice($id){
        ...
        $files = $this->purchase_model->get_purchase_invoice_attachments($id);
        $data['files'] = $files;
        ...
        $data['payment'] = $this->purchase_model->get_payment_approved_invoice($id);
    ...
    public function add_update_invoice($id = ''){
        ...
        if(get_option('allow_vendor_add_edit_delete_purchase_invoice') == 0){ 
            show_404();
        }
    ...
    public function pur_invoice_form(){
        ...
        if($this->input->post()){
            ...
            if($data['id'] == ''){
                ...
                if ($mess) {
                    handle_pur_invoice_file($mess, true);
                    set_alert('success', _l('added_successfully'));
                ...
                $id = $data['id'];
                unset($data['id']);
                handle_pur_invoice_file($id, true);
     ...
     * @param      string  $pur_order  The pur order
     */
    public function delete_pi_file($id, $pur_invoice){
        if (!is_vendor_logged_in()) {
            redirect(site_url('purchase/authentication_vendor/login'));
        }
        ...
        if($file->contact_id != $contact_id){
            set_alert('warning', _l('file_not_found'));
            redirect(site_url('purchase/vendors_portal/invoice/'.$pur_invoice.'?tab=attachment'));
        }

        $this->purchase_model->delete_purinv_attachment($id);
