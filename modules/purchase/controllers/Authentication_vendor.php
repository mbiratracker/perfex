...
{   
    ...
    {
        if(get_option('allow_vendors_to_register') != 1){
          show_404();
        }
        ...
        if ($this->input->post()) {
            if ($this->form_validation->run() !== false) {
                ...
                if ($clientid) {
                    ...
                    $this->purchase_model->require_confirmation($clientid);
                    $this->load->model('authentication_vendor_model');
    ...
    public function forgot_password()
    {
        ...
        if ($this->input->post()) {
            if ($this->form_validation->run() !== false) {
                ...
                redirect(site_url('purchase/authentication_vendor/forgot_password'));
            }
        }
        $data['title'] = _l('customer_forgot_password');
        $this->data($data);
        $this->view('vendor_portal/forgot_password');
    ...
    public function reset_password($staff, $userid, $new_pass_key)
    {
        ...
        $this->data($data);
        $this->view('vendor_portal/reset_password');
        $this->layout();
    }
