...
class Hr_profile extends AdminController {
	...
	public function test_imap_connection() {
		...
		require_once module_dir_path(HR_PROFILE_MODULE_NAME) . '/third_party/php-imap/Imap.php';
	...
	public function reception_staff() {
		...
		if(is_admin() || has_permission('hrm_hr_records','','view')){
			...
			$data['list_staff_not_record'] = $this->hr_profile_model->get_staff_active(false, 'active = 1 AND staffid not in (select staffid from '.db_prefix().'hr_rec_transfer_records)');
		}elseif(has_permission('hrm_hr_records','','view_own')){
	  		//View own
			$staff_ids = $this->hr_profile_model->get_staff_by_manager();
			if (count($staff_ids) > 0) {
				$data['list_staff_not_record'] = $this->hr_profile_model->get_staff(false, db_prefix().'staff.staffid IN (' . implode(', ', $staff_ids) . ') AND active = "1" AND staffid not in (select staffid from '.db_prefix().'hr_rec_transfer_records)');
			}else{
				$data['list_staff_not_record'] = [];
			}
		}else{
			$data['list_staff_not_record'] = [];
		}
	...
	public function training() {
		...
		if(is_admin() || has_permission('hrm_hr_records','','view')){
			...
			$data['list_staff'] = $this->hr_profile_model->get_staff_active();
		}elseif(has_permission('hrm_hr_records','','view_own')){
	  		//View own
			$staff_ids = $this->hr_profile_model->get_staff_by_manager();
			if (count($staff_ids) > 0) {
				$data['list_staff'] = $this->hr_profile_model->get_staff(false, db_prefix().'staff.staffid IN (' . implode(', ', $staff_ids) . ') AND active = "1"');
			}else{
				$data['list_staff'] = [];
			}
		}else{
		...
		if ($this->input->post()) {
			...
			if (!$this->input->post('id_training')) {
				...
				if ($id) {
					...
					$uploadedFiles = handle_hr_profile_training_program_attachments_array($id, 'file');

					if ($uploadedFiles && is_array($uploadedFiles)) {
						foreach ($uploadedFiles as $file) {
							$insert_file_id = $this->hr_profile_model->add_attachment_to_database($id, 'training_program', [$file]);
						}
					}
				...
				if (!empty($tmpFilePath) && $tmpFilePath != '') {
					...
					if (move_uploaded_file($tmpFilePath, $newFilePath)) {
						...
						$writer_header = array(
							...
							_l('passport_no') => 'string',
							_l('passport_expiry') => 'string',
							_l('emirates_ID_No') => 'string',
							_l('EID_expiry') => 'string',
							_l('labour_card_No') => 'string',
							_l('contact_person_home_country') => 'string',
							_l('home_contact_No') => 'string',
							_l('other_contact_person') => 'string',
							_l('other_contact_No') => 'string',
							_l('visa_validity') => 'string',
						...
						$widths = [40, 40, 40, 50, 40, 40, 40, 40, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50];
						...
						$arr_header['skype'] = 33;
						$arr_header['passport_no'] = 34;
						$arr_header['passport_expiry'] = 35;
						$arr_header['emirates_ID_No'] = 36;
						$arr_header['EID_expiry'] = 37;
						$arr_header['labour_card_No'] = 38;
						$arr_header['contact_person_home_country'] = 39;
						$arr_header['home_contact_No'] = 40;
						$arr_header['other_contact_person'] = 41;
						$arr_header['other_contact_No'] = 42;
						$arr_header['visa_validity'] = 43;

						$pattern = '#^[a-zA-Z0-9_.-]{2,31}@[a-z0-9\-]{3,}(\.[a-z]{2,4}){1,2}$#';
						...
						for ($row = 2; $row < count($data); $row++) {
							...
							$value_passport_no = isset($data[$row][$arr_header['passport_no']]) ? $data[$row][$arr_header['passport_no']] : '';
							$value_passport_expiry = isset($data[$row][$arr_header['passport_expiry']]) ? $data[$row][$arr_header['passport_expiry']] : '';
							$value_emirates_ID_No = isset($data[$row][$arr_header['emirates_ID_No']]) ? $data[$row][$arr_header['emirates_ID_No']] : '';
							$value_EID_expiry = isset($data[$row][$arr_header['EID_expiry']]) ? $data[$row][$arr_header['EID_expiry']] : '';
							$value_labour_card_No = isset($data[$row][$arr_header['labour_card_No']]) ? $data[$row][$arr_header['labour_card_No']] : '';
							$value_contact_person_home_country = isset($data[$row][$arr_header['contact_person_home_country']]) ? $data[$row][$arr_header['contact_person_home_country']] : '';
							$value_home_contact_No = isset($data[$row][$arr_header['home_contact_No']]) ? $data[$row][$arr_header['home_contact_No']] : '';
							$value_other_contact_person = isset($data[$row][$arr_header['other_contact_person']]) ? $data[$row][$arr_header['other_contact_person']] : '';
							$value_other_contact_No = isset($data[$row][$arr_header['other_contact_No']]) ? $data[$row][$arr_header['other_contact_No']] : '';
							$value_visa_validity = isset($data[$row][$arr_header['visa_validity']]) ? $data[$row][$arr_header['visa_validity']] : '';
							...
							if (is_null($value_email) == true || $value_email == '') {
								$string_error .= _l('client_email') . ' ' . _l('not_yet_entered') . '; ';
								$flag = 1;
							} else {
								if (preg_match($pattern, $value_email, $match) != 1) {
									$string_error .= _l('client_email') . ' ' . _l('invalid') . '; ';
									$flag = 1;
								} else {
									$flag_mail = 1;
								}
							...
							if ($flag_mail == 1) {
								...
								if ($value_staffid == '' || is_null($value_staffid) == true) {
									...
									if ($total_rows_email > 0) {
										$string_error .= _l('client_email') . ' ' . _l('exist') . '; ';
										$flag2 = 1;
									}
								}
							...
							if (is_null($value_job_position) != true && new_strlen($value_job_position) > 0) {
							...
							//value_team_manage
							if (is_null($value_team_manage) != true && new_strlen($value_team_manage) > 0) {
							...
							if (is_null($value_workplace) != true && new_strlen($value_workplace) > 0) {
							...
							//check role
							if (is_null($value_role) != true && new_strlen($value_role) > 0) {
							...
							//check department
							if (is_null($value_department) != true && new_strlen($value_department) > 0) {
								$arr_department_value = new_explode(';', $value_department);

								$deparments_flag = true;
								$str_deparments_not_exist = '';
								$temp_str_deparments_not_exist = new_explode(';', $value_department);
							...
							if (is_null($value_passport_expiry) != true && $value_passport_expiry != '') {

								if (is_null($value_passport_expiry) != true) {

									if (preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/", trim($value_passport_expiry, " "))) {
										$test = true;

									} else {
										$flag2 = 1;
										$string_error .= _l('hr_passport_expiry') . ' ' . _l('invalid') . '; ';
									}
								}
							}

							if (is_null($value_EID_expiry) != true && $value_EID_expiry != '') {

								if (is_null($value_EID_expiry) != true) {

									if (preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/", trim($value_EID_expiry, " "))) {
										$test = true;

									} else {
										$flag2 = 1;
										$string_error .= _l('hr_EID_expiry') . ' ' . _l('invalid') . '; ';
									}
								}
							}

							if (is_null($value_visa_validity) != true && $value_visa_validity != '') {

								if (is_null($value_visa_validity) != true) {

									if (preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/", trim($value_visa_validity, " "))) {
										$test = true;

									} else {
										$flag2 = 1;
										$string_error .= _l('hr_visa_validity') . ' ' . _l('invalid') . '; ';
									}
								}
							}
							...
							if (($flag == 1) || $flag2 == 1) {
								...
								$writer->writeSheetRow('Sheet1', [
									...
									$value_skype,
									$value_passport_no,
									$value_passport_expiry,
									$value_emirates_ID_No,
									$value_EID_expiry,
									$value_labour_card_No,
									$value_contact_person_home_country,
									$value_home_contact_No,
									$value_other_contact_person,
									$value_other_contact_No,
									$value_visa_validity,
									$string_error,
								]);
							...
							if ($flag == 0 && $flag2 == 0) {
								...
								if (new_strlen($value_password) > 0) {
								...
								$rd['passport_no'] = $value_passport_no;
								$rd['passport_expiry'] = $value_passport_expiry;
								$rd['emirates_ID_No'] = $value_emirates_ID_No;
								$rd['EID_expiry'] = $value_EID_expiry;
								$rd['labour_card_No'] = $value_labour_card_No;
								$rd['contact_person_home_country'] = $value_contact_person_home_country;
								$rd['home_contact_No'] = $value_home_contact_No;
								$rd['other_contact_person'] = $value_other_contact_person;
								$rd['other_contact_No'] = $value_other_contact_No;
								$rd['visa_validity'] = $value_visa_validity;
			...
			if (isset($_FILES['file_csv']['name']) && $_FILES['file_csv']['name'] != '') {
				...
				if (!empty($tmpFilePath) && $tmpFilePath != '') {
					...
					if (move_uploaded_file($tmpFilePath, $newFilePath)) {
						...
						$dataError->getActiveSheet()->setCellValue('D1', _l('client_email'));
						...
						foreach ($rowIterator as $row) {
							...
							if ($rowIndex > 1) {
								...
								if (($flag == 0) && ($flag2 == 0)) {
									...
									$rd['departments'] = new_explode(",", $sheet->getCell('V' . $rowIndex)->getValue());
		...
		foreach ($rResult as $aRow) {
			...
			if (isset($aRow['formality'])) {
				...
				if ($aRow['formality'] == 'indemnify') {
					...
					$t = new_explode(',', $aRow['formality_value']);
 ...
 */
		...
		if ($this->input->is_ajax_request()) {
			...
			foreach ($rResult as $aRow) {
				...
				if (has_permission('hrm_setting', '', 'delete')) {
					$options .= icon_btn('hr_profile/delete_hr_profile_permission/' . $aRow['staffid'], 'fa fa-remove', 'btn-danger _delete', ['title' => _l('delete')]);
				}
	...
	public function contracts($id = '') {
		...
		if(is_admin() || has_permission('hrm_hr_records','','view')){
			...
			$staff_ids = $this->hr_profile_model->get_staff_by_manager();
			if (count($staff_ids) > 0) {
				$data['staff'] = $this->hr_profile_model->get_staff(false, db_prefix().'staff.staffid IN (' . implode(', ', $staff_ids) . ')');
			}else{
				$data['staff'] = [];
			}
		}else{
	...
	public function contract($id = '') {
		...
		if(is_admin() || has_permission('hrm_hr_records','','view')){
			...
			$staff_ids = $this->hr_profile_model->get_staff_by_manager();
			if (count($staff_ids) > 0) {
				$data['staff'] = $this->hr_profile_model->get_staff(false, db_prefix().'staff.staffid IN (' . implode(', ', $staff_ids) . ') AND active = "1"');
			}else{
				$data['staff'] = [];
			}
		}else{
	...
	public function get_hrm_contract_data_ajax($id) {
		...
		if ($contract) {
			...
			if (new_strlen($contract->content) == 0) {
						...
						foreach ($rowIterator as $row) {
							...
							if ($rowIndex > 1) {
								...
								if (($flag == 0) && ($flag2 == 0)) {
									...
									$rd['dependent_bir'] = date('Y-m-d', strtotime(new_str_replace('/', '-', $sheet->getCell('D' . $rowIndex)->getValue())));
									...
									$rd['start_month'] = date('Y-m-d', strtotime(new_str_replace('/', '-', $sheet->getCell('G' . $rowIndex)->getValue())));
									$rd['end_month'] = date('Y-m-d', strtotime(new_str_replace('/', '-', $sheet->getCell('H' . $rowIndex)->getValue())));
	...
	public function dependent_person_modal() {
		...
		if(is_admin() || has_permission('hrm_hr_records','','view')){
			...
			$data['staff_members'] = $this->hr_profile_model->get_staff_active();
		}elseif(has_permission('hrm_hr_records','','view_own')){
	  		//View own
			$staff_ids = $this->hr_profile_model->get_staff_by_manager();
			if (count($staff_ids) > 0) {
				$data['staff_members'] = $this->hr_profile_model->get_staff(false, db_prefix().'staff.staffid IN (' . implode(', ', $staff_ids) . ') AND active = "1"');
			}else{
				$data['staff_members'] = [];
			}
		}else{
	...
	public function resignation_procedures() {
		...
		if(is_admin() || has_permission('hrm_hr_records','','view')){
			...
			$staff_ids = $this->hr_profile_model->get_staff_by_manager();
			if (count($staff_ids) > 0) {
				$data['staffs'] = $this->hr_profile_model->get_staff(false, db_prefix().'staff.staffid IN (' . implode(', ', $staff_ids) . ') AND active = "1"');
			}else{
				$data['staffs'] = [];
			}
		}else{
			...
			if ($staff) {
				...
				if (count($departments) > 0) {
					foreach ($departments as $value) {
						if (new_strlen($staff_department_name) > 0) {
...
// multiple deparment
	...
	public function view_training_program($id = '') {
		...
		$attachment = $this->hr_profile_model->get_training_program_attachment_html($id);
		$data['htmlfile'] = $attachment['htmlfile'];
	...
	public function create_staff_sample_file() {
		...
		$header_label = [
			...
			'hr_EID_expiry',
			'hr_labour_card_No',
			'hr_contact_person_home_country',
			'hr_home_contact_No',
			'hr_other_contact_person',
			'hr_other_contact_No',
			'hr_visa_validity',
		];
		...
		$writer->writeToFile(new_str_replace($filename, HR_PROFILE_CREATE_EMPLOYEES_SAMPLE . $filename, $filename));
	...
	public function preview_training_program_file($id, $rel_id) {
		...
		$this->load->view('hr_profile/training/preview_training_program_file', $data);
	}

	/**
	 * delete training program attachment file
	 * @param  [type] $attachment_id 
	 * @return [type]                
	 */
