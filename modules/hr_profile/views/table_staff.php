...
foreach ($rResult as $aRow) {
	...
	for ($i = 0; $i < count($aColumns); $i++) {
		...
		elseif($aColumns[$i] == 'sex'){
			if($aRow['sex'] != null && strlen($aRow['sex']) > 0){
				$_data = _l('hr_'.$aRow['sex']);
			}else{
				$_data = '';
			}
        
        }elseif($aColumns[$i] == 'status_work'){
        	if($aRow['status_work'] != null && strlen($aRow['status_work']) > 0){
				$_data = _l('hr_'.$aRow['status_work']);
			}else{
				$_data = '';
			}
		}         
		elseif ($aColumns[$i] == 'active') {
			...
			if (has_permission('hrm_hr_records', '', 'edit') || ($aRow['staffid'] == get_staff_user_id() && get_option('hide_general_staff_information') == 0) || is_admin()) {
