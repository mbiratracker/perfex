...
<div>
	<div class="_buttons">
		<?php if(is_admin() || has_permission('hrm_setting','','create')){ ?>
			...
			<a href="<?php echo admin_url('hr_profile/contract_template'); ?>" class="btn btn-primary pull-left display-block mright5"><?php echo _l('new_contract_template'); ?></a>
	...
	<table class="table dt-table">
		...
		<tbody>
			<?php foreach($contract_templates as $c){ ?>
				...
				<?php 
				$jobpositionOutput = '';
				$job_positions       = new_explode(",", $c['job_position']);
				...
				if($job_positions != null){
					foreach ($job_positions as $key => $position_id) {
						...
						$list_jobposition .= '<li class="text-success mbot10 mtop"><a href="#"  class="text-left">'.$position_name. '</a></li>';
				...
				<tr>
					...
					<td>
						<?php if(is_admin() || has_permission('hrm_setting','','edit')){ ?>
							<a href="<?php echo admin_url('hr_profile/contract_template/'.$c['id']); ?>"  class="btn btn-default btn-icon"><i class="fa-regular fa-pen-to-square"></i></a>
						<?php } ?>
