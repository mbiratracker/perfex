...
<div>
	<div class="_buttons">
		<?php if(has_permission('hrm_setting', '', 'create') || is_admin() ){ ?>
			<a href="#" onclick="new_allowance_type(); return false;" class="btn btn-primary pull-left display-block">
	...
	<table class="table dt-table">
		<thead>
			...
			<th><?php echo _l('hr_amount_of_money'); ?></th>
		...
		<tbody>
			<?php foreach($allowance_type as $c){ ?>
				<tr>
					<td><?php echo new_html_entity_decode($c['type_name']); ?></td>
					...
					<td>
						<?php if(has_permission('hrm_setting', '', 'edit') || is_admin()){ ?>
							<a href="#" onclick="edit_allowance_type(this,<?php echo new_html_entity_decode($c['type_id']); ?>); return false" data-taxable="<?php echo new_html_entity_decode($c['taxable']); ?>" data-name="<?php echo new_html_entity_decode($c['type_name']); ?>" data-amount="<?php echo new_html_entity_decode($c['allowance_val']); ?>" class="btn btn-default btn-icon"><i class="fa-regular fa-pen-to-square"></i></a>
						<?php } ?>
					...
					<div class="row">
						<div class="col-md-12">
							...
							<div class="form">     
								...
								echo render_input('allowance_val','hr_amount_of_money','','number',$arrAtt); ?> 
