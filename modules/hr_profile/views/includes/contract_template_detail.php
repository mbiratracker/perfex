						...
						<div class="modal-body">
							...
							<div class="tab-content">
								...
								<hr class="hr-color">
									...
									$arr_job_position = isset($contract_template) ? new_explode(",", $contract_template->job_position): [];
										...
										<div class="form-group">
											...
											<select name="job_position[]" id="job_position" data-live-search="true" class="selectpicker" multiple="true" data-actions-box="true" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
												<?php foreach($job_positions as $job_position) { 
													...
													?>
													<option value="<?php echo new_html_entity_decode($job_position['position_id']); ?>" <?php echo new_html_entity_decode($selected); ?>><?php echo new_html_entity_decode($job_position['position_name']); ?></option>
												<?php } ?>
											</select>
										</div>
									</div>
								...
								<div class="row">
									...
									<div class="row">
										<div class="col-md-8">
											...
											<?php } ?>
											<div class="contract-preview tc-content<?php if(has_permission('hrm_setting','','edit')){echo ' editable';} ?>"
												>
												<?php
												if((!isset($contract_template) || empty($contract_template->content) ) && has_permission('hrm_setting','','edit')){
													...
													echo new_html_entity_decode($contract_template->content);
												}
												?>
											</div>
										</div>
