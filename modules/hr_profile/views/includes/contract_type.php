...
<div>
	<div class="_buttons">
		<?php if(is_admin() || has_permission('hrm_setting','','create')){ ?>
			<a href="#" onclick="new_contract_type(); return false;" class="btn btn-primary pull-left display-block">
		...
		<tbody>
			<?php foreach($contract as $c){ ?>
				<tr>
					...
					if(new_strlen($c['description']) > 400){
						$description_sub = substr($c['description'],0, 400 ); 
					}else{
						$description_sub = $c['description'];
					}
					?>

					<td><?php echo new_html_entity_decode($c['name_contracttype']); ?></td>
					<td><?php echo new_html_entity_decode($description_sub); ?></td>
					<td>
						<?php if(is_admin() || has_permission('hrm_setting','','edit')){ ?>
							<a href="#" onclick="edit_contract_type(this,<?php echo new_html_entity_decode($c['id_contracttype']); ?>); return false"  class="btn btn-default btn-icon"><i class="fa-regular fa-pen-to-square"></i></a>
						<?php } ?>
