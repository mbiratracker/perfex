...
<div class="">
		...
		<table class="table border table-striped ">
			<tbody>
				...
				<tr class="project-overview">
					...
					<td class="text-right"><?php echo new_html_entity_decode($contract_code); ?></td>
				</tr>
				<tr class="project-overview">
					...
					<?php foreach($contract_type as $c){
						if(isset($contracts) && $contracts->name_contract == $c['id_contracttype'] ){
							?>
							<td class="text-right"><?php echo new_html_entity_decode($c['name_contracttype']); ?></td>
						<?php }?>
					<?php }?>
				</tr>
					...
					<?php foreach($staff as $s){
						if(isset($contracts) && $contracts->staff == $s['staffid'] ){
							...
							<td class="text-right">
								...
								</a><?php echo new_html_entity_decode($s['firstname'].' '.$s['lastname']); ?></td>
							<?php }?>
						<?php }?>
					</tr>
					<tr class="project-overview">
				...
				<tbody>
					...
					<tr class="project-overview">
						...
						if(isset($staff_delegate_role) && $staff_delegate_role != null){
							...
							<td class="text-right"><?php echo new_html_entity_decode($staff_role); ?></td>
	...
	<div role="tabpanel" class="tab-pane " id="contract">
		<div class="row">
			...
			<div class="col-md-12 text-right _buttons mbot5">
				<div class="btn-group">
				...
				<a href="<?php echo admin_url('hr_profile/contract_sign/'.$contracts->id_contract); ?>" class="btn btn-default "><span class="btn-with-tooltip" data-toggle="tooltip" data-title="<?php echo _l('View'); ?>" data-placement="bottom">
					...
					<?php }else{ ?>
						<div class="col-md-8">
					<?php } ?>
					<div class="contract-preview tw-overflow-auto tc-content<?php if(has_permission('hrm_contract','','edit')){echo ' editable';} ?>"
						>
						<?php
						if(empty($contracts->content) && has_permission('hrm_contract','','edit')){
							...
							echo new_html_entity_decode($contracts->content);
						}
						?>
					</div>
				</div>
							...
							<div class=" avilable_merge_fields mtop15 ">
								<ul class="list-group">
									...
									foreach($contract_merge_fields as $field){
										foreach($field as $f){
											echo '<li class="list-group-item tw-overflow-auto"><b>'.$f['name'].'</b>  <a href="javascript:void(0)" class="pull-right" onclick="insert_merge_field(this); return false">'.$f['key'].'</a></li>';
										}
									}
									?>
								</ul>
