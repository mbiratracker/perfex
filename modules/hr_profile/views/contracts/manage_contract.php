				...
				<div class="panel_s">
					...
					<div class="panel-body">
						<?php if(has_permission('hrm_contract','','create') || is_admin()){ ?>
							<div class="_buttons">
								<a href="<?php echo admin_url('hr_profile/contract'); ?>" class="btn btn-primary pull-left display-block mright5"><?php echo _l('new_contract'); ?></a>
							...
							<ul class="dropdown-menu dropdown-menu-left width300 height500">
								...
								<li class="dropdown-submenu pull-left">
									...
									<ul class="dropdown-menu dropdown-menu-left">
										<?php  foreach($staff as $s){ ?>
											<li><a href="#" data-cview="contracts_by_staff_<?php echo new_html_entity_decode($s['staffid']); ?>" onclick="dt_custom_view('contracts_by_staff_<?php echo new_html_entity_decode($s['staffid']); ?>','.table-table_contract','contracts_by_staff_<?php echo new_html_entity_decode($s['staffid']); ?>'); return false;">
								...
								<?php if(count($contract_type) > 0){ ?>
									...
									<?php foreach($contract_type as $type){ ?>
										<li>
											<a href="#" data-cview="contracts_by_type_<?php echo new_html_entity_decode($type['id_contracttype']); ?>" onclick="dt_custom_view('contracts_by_type_<?php echo new_html_entity_decode($type['id_contracttype']); ?>','.table-table_contract','contracts_by_type_<?php echo new_html_entity_decode($type['id_contracttype']); ?>'); return false;">
												<?php echo new_html_entity_decode($type['name_contracttype']); ?>
											</a>
										</li>
									<?php } ?>
								<?php } ?>
								...
								<?php if(count($duration) > 0){ ?>
									...
									<?php foreach($duration as $type){ ?>
										<li class="filter-group" data-filter-group="duration">
											<a href="#" data-cview="contracts_by_duration_<?php echo new_html_entity_decode($type['duration']).'_'.$type['unit']; ?>" onclick="dt_custom_view('contracts_by_duration_<?php echo new_html_entity_decode($type['duration']).'_'.$type['unit']; ?>','.table-table_contract','contracts_by_duration_<?php echo new_html_entity_decode($type['duration']).'_'.$type['unit']; ?>'); return false;">
												<?php echo new_html_entity_decode($type['duration']).' '.$type['unit']; ?>
											</a>
											<li>
											<?php } ?>
										<?php } ?>
										...
										<div class="modal-dialog" role="document">
											<div class="modal-content">
												...
												<div class="modal-footer">
													...
													<?php if(has_permission('hrm_contract','','delete') || is_admin()){ ?>
														<a href="#" class="btn btn-primary" onclick="staff_delete_bulk_action(this); return false;"><?php echo _l('hr_confirm'); ?></a>
													<?php } ?>
												</div>
											</div>
										</div>
