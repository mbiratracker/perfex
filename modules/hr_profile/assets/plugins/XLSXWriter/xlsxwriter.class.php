...
class XLSXWriter
{
	...
	public function writeSheetHeader_v2($sheet_name, array $header_types, $col_options = null, $col_style1 = [], $style1 = null, $col_style2 = [], $style2 = null)
	{
		if (empty($sheet_name) || empty($header_types) || !empty($this->sheets[$sheet_name]))
			return;
		...
		if (!$suppress_row)
		{
			...
			foreach ($header_row as $c => $v) {
				...
				if(in_array($c, $col_style1)){
					...
					$this->writeCell($sheet->file_writer, 0, $c, $v, $number_format_type='n_string', $cell_style_idx);
				}elseif(in_array($c, $col_style2)){
					$cell_style_idx = empty($style2) ? $sheet->columns[$c]['default_cell_style'] : $this->addCellStyle( 'GENERAL', json_encode(isset($style2[0]) ? $style2[$c] : $style2) );
