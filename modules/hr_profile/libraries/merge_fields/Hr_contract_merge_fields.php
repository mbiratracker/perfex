...
class Hr_contract_merge_fields extends App_merge_fields
{
	public function build()
	{
		return [
			...
			[
				'name'      => 'Staff Bank Account Number',
				'key'       => '{account_number}',
				'available' => [
					'hr_contract',
				],
	...
	public function format($contract_id)
	{
		...
		$fields['{account_number}'] 			=  $contract->account_number;
		$fields['{name_account}'] 				=  $contract->name_account;
		$fields['{issue_bank}'] 				=  $contract->issue_bank;
		...
		$fields['{facebook}'] 			=  $contract->facebook;
		$fields['{linkedin}'] 			=  $contract->linkedin;
		$fields['{skype}'] 			=  $contract->skype;
		$fields['{place_of_issue}'] 			=  $contract->place_of_issue;
		$fields['{team_manage}'] 			=  (!is_null($contract->team_manage) && is_numeric($contract->team_manage)) ? get_staff_full_name($contract->team_manage) : '' ;
		$fields['{literacy}'] 			=  !is_null($contract->literacy) ? _l('hr_'.$contract->literacy) : '';
		$fields['{hourly_rate}'] 			=  $contract->hourly_rate;
		$fields['{default_language}'] 			=  (!is_null($contract->default_language) && $contract->default_language != '') ? $contract->default_language : get_option('active_language');
		$fields['{direction}'] 			=  $contract->direction;
		$fields['{orther_infor}'] 			=  $contract->orther_infor;
		$fields['{marital_status}'] 			=  $contract->marital_status;
