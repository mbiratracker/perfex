...
class Hr_profile_model extends App_Model
{
	...
	{
		$contracts = $this->db->query('SELECT * FROM '.db_prefix().'hr_staff_contract')->result_array();
	...
	*/
	...
	public function get_data_chart()
	{
		$department =  $this->db->query('select s.staffid as id, s.team_manage as pid, CONCAT(s.firstname," ",s.lastname) as name,  r.name as rname, j.position_name as job_position_name
			from '.db_prefix().'staff as s left join '.db_prefix().'roles as r on s.role = r.roleid left join '.db_prefix().'hr_job_position as j on j.position_id = s.job_position where s.status_work != "inactivity"       
	...
	public function hr_profile_get_department_name($departmentid){
		return $this->db->query('select '.db_prefix().'departments.name from '.db_prefix().'departments where departmentid = '.$departmentid)->row();
	}
	/**
 ...
 public function get_job_position_interview_process($id = false){
   ...
   */
	public function get_job_p($id = false)
	{
		...
		if ($id == false) {
			return $this->db->query('select * from '.db_prefix().'hr_job_p')->result_array();
		}
	}
		...
		if(count($job_position) > 0){
			foreach ($job_position as $job_value) {
				if($job_value['department_id'] != null && $job_value['department_id'] != ''){
				 ...
				 $arr = new_explode(',', $job_value['department_id']);
...
public function get_position_by_department($department_id, $status)
{
	...
	public function add_training_staff($data_training,$id_staff){
		...
		$explode = new_explode(',', $data_training->position_training_id);
...
public function get_job_position_arrayid()
{
	...
	{
		$workplace = $this->db->query('select * from '.db_prefix().'hr_workplace')->result_array();
		$workpalce_array =[];
 ...
 public function format_date($date){
	...
	public function add_staff($data)
	{
		...
		if (isset($data['EID_expiry'])) {
			$data['EID_expiry'] = to_sql_date($data['EID_expiry']);
		}else{
			$data['EID_expiry'] = null;
		}
		if (isset($data['visa_validity'])) {
			$data['visa_validity'] = to_sql_date($data['visa_validity']);
		}else{
			$data['visa_validity'] = null;
		}
	...
	*/
	...
	{
		return $this->db->query('select '.db_prefix().'staff.records_received from '.db_prefix().'staff where staffid = '.$id)->row();
	}
	...
	public function get_duration(){
		return $this->db->query('SELECT duration, unit FROM '.db_prefix().'hr_staff_contract_type group by duration, unit')->result_array();
	}
...
public function member_get_evaluate_form($staffid){
	...
	foreach ($arr_evaluate_form as $evaluate_value) {
		$data =[];
		if(new_strlen(json_encode($arr_staff)) != 2){
...
public function get_dataobject_result_evaluate($id, $arrstaff = false){
	...
	if($arrstaff != false){
		...
		if(isset($evaluation_form->department_id) && $evaluation_form->department_id != 'null' && $evaluation_form->department_id != '0'&&$evaluation_form->apply_for=='department'){
			$searchVal = array('[', ']', '"');
			$replaceVal = array('(', ')', '');
			$department_array = new_str_replace($searchVal, $replaceVal, $evaluation_form->department_id);
			$sql .= ' AND staffid in ( select staffid from '.db_prefix().'staff_departments where departmentid in '.$department_array.' )';
		}
		if(isset($evaluation_form->role_id) && $evaluation_form->role_id != 'null' && $evaluation_form->role_id != '0'&&$evaluation_form->apply_for=='role'){
		 $searchVal = array('[', ']', '"');
		 $replaceVal = array('(', ')', '');
		 $role_array = new_str_replace($searchVal, $replaceVal, $evaluation_form->roles_id);
	 ...
	 if(isset($evaluation_form->staff_id) && $evaluation_form->staff_id != 'null' && $evaluation_form->staff_id != '0'&&$evaluation_form->apply_for=='staff'){
		 $searchVal = array('[', ']', '"');
		 $replaceVal = array('(', ')', '');
		 $staff_array = new_str_replace($searchVal, $replaceVal, $evaluation_form->staff_id);
					...
					if($evaluate_result_value['assessor_id'] == $staff_id){
						...
						foreach ($arr_result as $arr_result_value) {
							if($arr_result_value->staff_id == $staff['staff_identifi']){
								...
								$formula = new_str_replace($kpi_value,$result_value,$formula);
...
public function add_attachment_to_database($rel_id, $rel_type, $attachment, $external = false)
{
   ...
   public function get_staff_in_deparment($department_id)
   {
	...
	public function get_data_dpm_chart($dpm)
	{
		...
		from '.db_prefix().'staff as s 
		 left join '.db_prefix().'staff_departments as sd on sd.staffid = s.staffid
				left join '.db_prefix().'departments d on d.departmentid = sd.departmentid where d.departmentid = "'.$dpm.'" and s.status_work != "inactivity"
		order by s.team_manage, s.staffid')->result_array();
	...
	public function delete_job_position_training_process($trainingid){
		...
		//delete atachement file
		$training_program_file = $this->get_hr_profile_file($trainingid, 'training_program');
		foreach ($training_program_file as $file_key => $file_value) {
			$this->delete_training_program_attachment_file($file_value['id']);
		}
		//delete general info
	...
	public function update_prefix_number($data)
	{
		...
		$hide_general_staff_information = 0;
		...
		if(isset($data['hide_general_staff_information'])){
			$hide_general_staff_information = $data['hide_general_staff_information'];
			unset($data['hide_general_staff_information']);
		}
		$update_option_re =  update_option('hide_general_staff_information', $hide_general_staff_information);
		if($update_option_re){
			$affected_rows++;
		}
	...
	public function check_department_format($departments)
	{
		...
		$arr_department = new_explode(',', $departments);
		...
		if(count($contract_details) > 0){
			foreach ($contract_details as $key => $value) {
				...
				if(preg_match('/^st_/', $value['rel_type'])){
					$rel_value = new_str_replace('st_', '', $value['rel_type']);
					...
					$rel_value = new_str_replace('al_', '', $value['rel_type']);
	...
	public function get_staff_from_training_program($training_programs)
	{
		...
		foreach ($training_programs as $training_program) {
		    if($training_program['additional_training'] == 'additional_training'){
				$training_program_staff=new_explode(',', $training_program['staff_id']);
				...
				if ($for_editor == false) {
					...
					foreach ($merge_fields as $key => $val) {
						...
						if($key == '{logo_image_with_url}'){
							...
							$val .= '<a href="'.$logo_url.'" class="logo hr-img-responsive" >';
							$val .= '<img src="'.$logo_url.'" class="hr-img-responsive" width="250" height="auto" alt="GTSS Solution Viet Nam">';
							$val .= '</a>';
					...
					$contract->content .= '<div class="col-md-6  text-left">';
						...
						$contract->content .= '<div class="pull-left">';
							if(new_strlen($contract->staff_signature) > 0){
					...
					$contract->content .= '<div class="col-md-6  text-right">';
						...
						$contract->content .= '<div class="pull-right">';
							if(new_strlen($contract->signature) > 0){
	...
	/**
	 * get training program attachment html
	 * @param  [type] $training_program_id 
	 * @return [type]                      
	 */
	public function get_training_program_attachment_html($training_program_id)
	{
		$arr_file = $this->get_hrm_attachments_file($training_program_id, 'training_program');
		$htmlfile='';
		//get file attachment html
		if(isset($arr_file)){
			$htmlfile = '<div class="row col-md-12" id="attachment_file">';
			foreach($arr_file as $attachment) {
				$href_url = site_url('modules/hr_profile/uploads/training_programs/'.$attachment['rel_id'].'/'.$attachment['file_name']).'" download';
				if(!empty($attachment['external'])){
					$href_url = $attachment['external_link'];
				}
				...
				$htmlfile .= '<div class="col-md-10">';
				$htmlfile .= '<div class="col-md-1 mr-5">';
				$htmlfile .= '<a name="preview-btn" onclick="preview_training_program_file(this); return false;" rel_id = "'.$attachment['rel_id'].'" id = "'.$attachment['id'].'" href="Javascript:void(0);" class="mbot10 btn btn-success pull-left" data-toggle="tooltip" title data-original-title="'._l("preview_file").'">';
				$htmlfile .= '<i class="fa fa-eye"></i>'; 
				$htmlfile .= '</a>';
				$htmlfile .= '</div>';
				$htmlfile .= '<div class=col-md-9>';
				...
				if(has_permission('staffmanage_training', '', 'delete')){
					$htmlfile .= '<a href="#" class="text-danger" onclick="delete_training_program_attachment(this,'.$attachment['id'].'); return false;"><i class="fa fa fa-times"></i></a>';
				}
	...
	public function delete_training_program_attachment_file($attachment_id)
	{
		...
		if ($attachment) {
			if (empty($attachment->external)) {
				unlink(get_hr_profile_upload_path_by_type('training_program') .$attachment->rel_id.'/'.$attachment->file_name);
			}
			$this->db->where('id', $attachment->id);
			$this->db->delete(db_prefix() . 'files');
			if ($this->db->affected_rows() > 0) {
				$deleted = true;
				log_activity('Training Program Attachment Deleted [training_programID: ' . $attachment->rel_id . ']');
			}

			if (is_dir(get_hr_profile_upload_path_by_type('training_program') .$attachment->rel_id)) {
				// Check if no attachments left, so we can delete the folder also
				$other_attachments = list_files(get_hr_profile_upload_path_by_type('training_program') .$attachment->rel_id);
				if (count($other_attachments) == 0) {
					// okey only index.html so we can delete the folder also
					delete_dir(get_hr_profile_upload_path_by_type('training_program') .$attachment->rel_id);
				}
			}
		}
