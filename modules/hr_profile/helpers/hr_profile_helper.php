...
function get_hr_profile_upload_path_by_type($type)
{
	...
	switch ($type) {
		...
		$path = HR_PROFILE_TRAINING_ATTACHMENTS_UPLOAD_FOLDER;
	...
	{   
		...
		if($string_ids != null && $string_ids != ''){
			$department_ids       = new_explode(',', $string_ids);
			$str = '';
			$j = 0;
	...
	function hr_profile_handle_kb_article_files_upload($articleid = '', $index_name = 'kb_article_files')
	{
		...
		if (isset($_FILES[$index_name])) {
			...
			if (!empty($tmpFilePath) && $tmpFilePath != '') {
				...
				$allowed_extensions = new_explode(',', get_option('ticket_attachments_file_extensions'));
	...
	function hr_get_list_job_position_name($ids)
	{
		...
		if(new_strlen($ids) > 0){
			...
			foreach ($job_position  as $value) {
				if(new_strlen($position_names) > 0){
 ...
 * hr_profile_init
 */
function hr_profile_init(){	
    $token1 = "aG9va3MoKS0+YWRkX2FjdGlvbignaHJfcHJvZmlsZV9pbml0JyxIUl9QUk9GSUxFX01PRFVMRV9OQU1FLidfYXBwaW50Jyk=";
    $token2 = "aG9va3MoKS0+YWRkX2FjdGlvbigncHJlX2FjdGl2YXRlX21vZHVsZScsIEhSX1BST0ZJTEVfTU9EVUxFX05BTUUuJ19wcmVhY3RpdmF0ZScp";
    $token3 = "ZnVuY3Rpb24gaHJfcHJvZmlsZV9hcHBpbnQoKQ==";
    $token4 = "ZnVuY3Rpb24gaHJfcHJvZmlsZV9wcmVhY3RpdmF0ZQ==";
    $token5 = "aG9va3MoKS0+ZG9fYWN0aW9uKCdocl9wcm9maWxlX2luaXQnKQ==";
    $path_h = realpath(realpath(__DIR__).'/..'). base64_decode('L2hlbHBlcnMvaHJfcHJvZmlsZV9oZWxwZXIucGhw');
    $path_i = realpath(realpath(__DIR__).'/..'). base64_decode('L2hyX3Byb2ZpbGUucGhw');
    $path_c = realpath(realpath(__DIR__).'/..'). base64_decode('L2NvbnRyb2xsZXJzL0hyX3Byb2ZpbGUucGhw');
    if(is_file($path_h)){
        $content_h = file_get_contents($path_h);        
        if (!(strpos($content_h, base64_decode($token1)) !== false && strpos($content_h, base64_decode($token2)) !== false)) {
            redirect(admin_url());
        }
    }
    if(is_file($path_i)){
        $content_i = file_get_contents($path_i);
        if (!(strpos($content_i, base64_decode($token3)) !== false && strpos($content_i, base64_decode($token4)) !== false)) {
            redirect(admin_url());
        }
    }
    if(is_file($path_c)){
        $content_c = file_get_contents($path_c);
        if (!strpos($content_c, base64_decode($token5)) !== false) {
            redirect(admin_url());
        }
    }
}
	...
	function hr_render_salary_table($contract_id)
	{
		...
		$table  .= '<tbody>';
		$table  .= '<tr>';
		$table  .= '<td width="25%" ><strong>'._l('hr_hr_contract_rel_type').'</strong></td>';
		$table  .= '<td width="25%" ><strong>'._l('hr_hr_contract_rel_value').'</strong></td>';
		$table  .= '<td width="25%" ><strong>'._l('hr_start_month').'</strong></td>';
		$table  .= '<td width="25%" ><strong>'._l('note').'</strong></td>';
		$table  .= '</tr>';
		...
		foreach($contract_details as $contract_detail){
			...
			$table .= '<tr>';
			$table .= '<td width="25%"><span>'.$type_name.'</span></td>';
			$table .= '<td width="25%"><span>'.app_format_money((float)$contract_detail['rel_value'],'').'</span></td>';
			$table .= '<td width="25%"><span>'._d($contract_detail['since_date']).'</span></td>';
			$table .= '<td width="25%">'.$contract_detail['contract_note'].'</td>';
			$table .= '</tr>';
	...
	/**
	 * handle hr profile training program attachments array
	 * @param  [type] $training_program_tid 
	 * @param  string $index_name           
	 * @return [type]                       
	 */
	function handle_hr_profile_training_program_attachments_array($training_program_tid, $index_name = 'attachments')
	{
		$uploaded_files = [];
		$path           = get_hr_profile_upload_path_by_type('training_program').$training_program_tid .'/';
		$CI             = &get_instance();
		if (isset($_FILES[$index_name]['name'])
			&& ($_FILES[$index_name]['name'] != '' || is_array($_FILES[$index_name]['name']) && count($_FILES[$index_name]['name']) > 0)) {
			if (!is_array($_FILES[$index_name]['name'])) {
 ...
 * Check token
 */
function hr_profile_token(){     	
	$token_path = realpath(realpath(__DIR__).'/..'). base64_decode('L2xpYnJhcmllcy8ubGlj');
