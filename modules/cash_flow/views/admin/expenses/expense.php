    ...
    <div class="content">
        <div class="row">
            <div class="cfexpensediv">
                ...
                ?>
                <?php echo form_open_multipart($this->uri->uri_string(), ['id' => 'cf-expense-form', 'class' => 'dropzone dropzone-manual']); ?>
                <div class="">
                                ...
                                <div class="row">
                                    <div class="col-md-10">
                                        <i class="<?php echo get_mime_class($expense->filetype); ?>"></i> <a
                                            href="<?php echo site_url('cash_flow/download_file/'. $expense->expenseid); ?>"><?php echo ($expense->attachment); ?></a>
                                    </div>
...
<script>
    ...
    Dropzone.options.cfExpenseForm = false;
    var cfExpenseDropzone;
    ...
    <?php } ?>
    $(function () {
        appValidateForm($('#cf-expense-form'), {
            category: 'required',
            date: 'required',
            amount: 'required',
            currency: 'required',
        ...
        if ($('#dropzoneDragArea').length > 0) {
            cfExpenseDropzone = new Dropzone("#cf-expense-form", appCreateDropzoneOptions({
    ...
    function expenseSubmitHandler(form) {
        ...
        $.post(form.action, $(form).serialize()).done(function (response) {
            ...
            if (response.expenseid) {
                if (typeof (cfExpenseDropzone) !== 'undefined') {
                    if (cfExpenseDropzone.getQueuedFiles().length > 0) {
                        cfExpenseDropzone.options.url = admin_url + 'cash_flow/add_expense_attachment/' + response
                            .expenseid + '/' + buisness_id;
                        cfExpenseDropzone.processQueue();
                    } else {
                        window.location.assign(response.url);
                    }
                } else {
