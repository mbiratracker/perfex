...
define('MANUFACTURING_ORDER_UPLOAD_FOLDER', module_dir_path(MANUFACTURING_MODULE_NAME, 'uploads/manufacturing_orders/'));
...
hooks()->add_action('task_modal_rel_type_select', 'workorder_task_modal_rel_type_select');
hooks()->add_filter('relation_values', 'workorder_get_relation_values', 10, 2);
hooks()->add_filter('get_relation_data', 'workorder_get_relation_data', 10, 4);
hooks()->add_filter('tasks_table_row_data', 'workorder_add_table_row', 10, 3);

define('VERSION_MANUFACTURING', 1102);
...
{   
	...
	function manufacturing_load_js(){    
		...
		if(!(strpos($viewuri, '/admin/manufacturing') === false)){
			echo '<script src="' . module_dir_url(MANUFACTURING_MODULE_NAME, 'assets/js/tinymce_init.js') .'?v=' . VERSION_MANUFACTURING.'"></script>';
         echo '<script src="' . module_dir_url(MANUFACTURING_MODULE_NAME, 'assets/plugins/signature_pad.min.js') . '"></script>';
...
function manufacturing_appint(){
    ...
    if(!$manufacturing_gtssres || ($manufacturing_gtssres && isset($manufacturing_gtssres['status']) && !$manufacturing_gtssres['status'])){
        ...
        set_alert('danger', base64_decode('T25lIG9mIHlvdXIgbW9kdWxlcyBmYWlsZWQgaXRzIHZlcmlmaWNhdGlvbiBhbmQgZ290IGRlYWN0aXZhdGVkLiBQbGVhc2UgcmVhY3RpdmF0ZSBvciBjb250YWN0IHN1cHBvcnQu'));
