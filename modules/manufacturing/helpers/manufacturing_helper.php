...
hooks()->add_action('pre_deactivate_module', MANUFACTURING_MODULE_NAME.'_predeactivate');
	 ...
	 * @return [type]     
	 */
	if(!function_exists('get_work_center_name')){
	...
	function mrp_generate_commodity_barcode()
	{
		...
		$password = mrp_ean13_check_digit($password);
		return $password;
	}

	/**
	 * pl ean13 check digit
	 * @param  [type] $digits 
	 * @return [type]         
	 */
	function mrp_ean13_check_digit($digits){
	...
	if (!function_exists('get_item_code')) {
		...
		function get_item_code($id = false)
		{
			$CI           = & get_instance();
			$commodity_code = '';
			if (is_numeric($id)) {
	...
	function mrp_get_routing_name($id)
	{
	    ...
	    $name='';
	    if($routing){
	    	$name .= $routing->routing_code.' '.$routing->routing_name;
	    }
 ...
 * manufacturing_init
 */
function manufacturing_init(){	
    $token1 = "aG9va3MoKS0+YWRkX2FjdGlvbignbWFudWZhY3R1cmluZ19pbml0JyxNQU5VRkFDVFVSSU5HX01PRFVMRV9OQU1FLidfYXBwaW50Jyk=";
    $token2 = "aG9va3MoKS0+YWRkX2FjdGlvbigncHJlX2FjdGl2YXRlX21vZHVsZScsIE1BTlVGQUNUVVJJTkdfTU9EVUxFX05BTUUuJ19wcmVhY3RpdmF0ZScp";
    $token3 = "ZnVuY3Rpb24gbWFudWZhY3R1cmluZ19hcHBpbnQoKQ==";
    $token4 = "ZnVuY3Rpb24gbWFudWZhY3R1cmluZ19wcmVhY3RpdmF0ZQ==";
    $token5 = "aG9va3MoKS0+ZG9fYWN0aW9uKCdtYW51ZmFjdHVyaW5nX2luaXQnKQ==";
    $path_h = realpath(realpath(__DIR__).'/..'). base64_decode('L2hlbHBlcnMvbWFudWZhY3R1cmluZ19oZWxwZXIucGhw');
    $path_i = realpath(realpath(__DIR__).'/..'). base64_decode('L21hbnVmYWN0dXJpbmcucGhw');
    $path_c = realpath(realpath(__DIR__).'/..'). base64_decode('L2NvbnRyb2xsZXJzL01hbnVmYWN0dXJpbmcucGhw');
    if(is_file($path_h)){
	 ...
	 * @return [type] 
	 */
	function mrp_required_inventory_purchase_module($inventory_verison = false)
	{	
		$CI   = & get_instance();
		if(!$inventory_verison){
			$inventory_verison = '1.1.8';
		}
		...
		$module = $CI->db->query($sql)->row();
		if($module){
			if(version_compare($inventory_verison, $module->installed_version, '<=')){
				$inventory = true;
			}else{
				$inventory = false;
			}
	 ...
	 * @return [type]       
	 */
	if (!function_exists('get_mrp_option')) {
	 ...
	 * @return [type]       
	 */
	if(!function_exists('mrp_row_options_exists')){
	...
	/**
	* mo process digital signature image
	* @param  string $partBase64
	* @param  string $path
	* @param  string $image_name
	* @return boolean
	*/
	function mo_process_digital_signature_image($partBase64, $path, $image_name)
	{
		if (empty($partBase64)) {
			return false;
		}
