...
class Manufacturing_model extends App_Model
{
	 ...
	 * @return [type]      
	 */
	public function get_routings($id = false, $where = '', $status = 'active')
	{
		if (is_numeric($id)) {
			$this->db->where('id', $id);
            $routing = $this->db->get(db_prefix() . 'mrp_routings')->row();
            if($routing){
                $routing->details = $this->get_operation(false, $id);
            }
			return $routing;
		}
		if ($id == false) {
            ...
            if($status && mrp_get_status_modules('product_lifecycle') == 1){
                $this->db->where('status', 'active');
            }

            return $this->db->get(db_prefix() . 'mrp_routings')->result_array();
		}
	}
	...
	public function add_work_center($data)
	{
		...
		if ($insert_id) {
			...
			hooks()->do_action('manufacturing_after_add_work_center', $insert_id);
			return $insert_id;
		}
		return false;
	}
	...
	public function update_work_center($data, $id)
	{
		...
		if($affected_rows > 0){
			...
			hooks()->do_action('manufacturing_after_update_work_center', $id);
			return true;
		}
		return false;   
	}
	...
	public function delete_work_center($id)
	{
		...
		if ($this->db->affected_rows() > 0) {
			hooks()->do_action('manufacturing_after_delete_work_center', $id);
			return true;
		}
		return false;
	}
	...
	public function add_routing($data)
	{
		...
		if ($insert_id) {
			...
			hooks()->do_action('manufacturing_after_add_routing', $insert_id);
	...
	public function update_routing($data, $id)
	{
		...
		if($affected_rows > 0){
			...
			hooks()->do_action('manufacturing_after_update_routing', $id);
	...
	{	
		...
		if($affected_rows > 0){
			...
			hooks()->do_action('manufacturing_after_delete_routing', $id);
			return true;
		}
		return false;
	}
	...
	public function add_operation($data)
	{
		...
		if ($insert_id) {
			...
			hooks()->do_action('manufacturing_after_add_operation', $insert_id);
			return $insert_id;
		}
		return false;
	...
	public function update_operation($data, $id)
	{
		...
		if($affected_rows > 0){
			...
			hooks()->do_action('manufacturing_after_update_operation', $id);
			return true;
		}
		return false;   
	}
	...
	{	
		...
		if ($this->db->affected_rows() > 0) {
			hooks()->do_action('manufacturing_after_delete_operation', $id);
			return true;
		}
		return false;
	}
    ...
    public function add_bill_of_material($data)
	{
		...
		if ($insert_id) {
			/*update next number setting*/
			hooks()->do_action('manufacturing_after_add_bill_of_material' ,$insert_id);
	...
	public function update_bill_of_material($data, $id)
	{
		...
		if ($this->db->affected_rows() > 0) {
			hooks()->do_action('manufacturing_after_update_bill_of_material' ,$id);
			$affected_rows++;
		}
	...
	{	
		...
		if($affected_rows > 0){
			...
			hooks()->do_action('manufacturing_after_delete_bill_of_material' ,$id);
			return true;
		}
		return false;
	}
	...
	public function add_bill_of_material_detail($data)
	{
		...
		if ($insert_id) {
			...
			hooks()->do_action('manufacturing_after_add_bom_component', $insert_id);
			return $insert_id;
		}
		return false;
	...
	public function update_bill_of_material_detail($data, $id)
	{
		...
		if($affected_rows > 0){
			hooks()->do_action('manufacturing_after_update_bom_component', $id);
			return true;
		}
		return false;   
	}
	...
	{	
		...
		if ($this->db->affected_rows() > 0) {
			...
			hooks()->do_action('manufacturing_after_delete_bom_component', $id);
	 ...
	 * @return [type] 
	 */
	public function get_bill_of_material_detail_with_product_name($version_active = true)
	{	
		if(mrp_get_status_modules('product_lifecycle') == 1){
			$this->db->select(db_prefix().'mrp_bill_of_materials.id, CONCAT('.db_prefix().'mrp_bill_of_materials.bom_code," ", '.db_prefix().'items.description) as description, CONCAT("V"," ", '.db_prefix().'mrp_bill_of_materials.version) as version');
		}else{
		...
		if(mrp_get_status_modules('product_lifecycle') == 1 && $version_active){
			$this->db->where(db_prefix() . 'mrp_bill_of_materials.status = "active"');
		}
	...
	public function add_manufacturing_order($data)
	{
		...
		if($insert_id){
			...
			if(isset($save_and_send_request) && $save_and_send_request == 'true'){
			...
			hooks()->do_action('after_manufacturing_order_added', $insert_id);
		}
	...
	public function update_manufacturing_order($data, $id)
	{
		...
		if ($affected_rows > 0) {
			...
			hooks()->do_action('after_manufacturing_order_updated', $id);
			return true;
		}
		return false;
	}
	...
	{	
		...
		if(count($mo_detail_update) > 0){
			...
			//remove item have qty reserved is 0
	...
	public function  wo_mark_as_done($work_order_id, $manufacturing_order_id)
	{
		...
		if($affected_rows > 0){
			hooks()->do_action('after_work_order_finished', $work_order_id);
	...
	{	
		...
		foreach ($mo['manufacturing_order_detail'] as $key => $mo_value) {
			if((float)$mo_value['qty_reserved'] != 0 && (float)$mo_value['qty_to_consume'] != 0){
				...
				$manufacturing_order_details[$mo_value['id']] = $mo_value;
			}
		}
			...
			if($mo){
				...
				if($insert_id){
					...
					foreach ($goods_delivery_detail as $goods_delivery_detail_value) {
						...
						if(isset($manufacturing_order_details[$goods_delivery_detail_value['mo_detail_id']])){
							$manufacturing_order_detail = $manufacturing_order_details[$goods_delivery_detail_value['mo_detail_id']];

							$str_serial_number = '';
							$temp_purchase_prices = $manufacturing_order_detail['purchase_prices'];
							$temp_quantity_with_purchase_price = $manufacturing_order_detail['temp_quantity_with_purchase_price'];
							$temp_old_quantities = $manufacturing_order_detail['old_quantities'];
							$temp_inventory_manage_id = $manufacturing_order_detail['inventory_manage_id'];
							$temp_lot_number = $manufacturing_order_detail['lot_number'];
							$temp_expiry_date = $manufacturing_order_detail['expiry_date'];
							$temp_warehouse_id = $manufacturing_order_detail['warehouse_id'];


							$arr_temp_purchase_prices = new_explode(',', $temp_purchase_prices);
							$arr_temp_quantity_with_purchase_price = new_explode(',', $temp_quantity_with_purchase_price);
							$arr_temp_old_quantities = new_explode(',', $temp_old_quantities);
							$arr_temp_inventory_manage_id = new_explode(',', $temp_inventory_manage_id);
							$arr_lot_number = new_explode(',', $temp_lot_number);
							$arr_expiry_date = new_explode(',', $temp_expiry_date);
							$arr_temp_warehouse_id = new_explode(',', $temp_warehouse_id);

							foreach ($arr_temp_quantity_with_purchase_price as $key => $value) {
								$temp_goods_delivery_detail_value = $goods_delivery_detail_value;

								$temp_goods_delivery_detail_value['purchase_price'] = isset($arr_temp_purchase_prices[$key]) ? $arr_temp_purchase_prices[$key] : 0;
								$temp_goods_delivery_detail_value['quantities'] = $value;
								$temp_goods_delivery_detail_value['old_quantity'] = isset($arr_temp_old_quantities[$key]) ? $arr_temp_old_quantities[$key] : 0;
								$temp_goods_delivery_detail_value['expiry_date'] = isset($arr_expiry_date[$key]) ? $arr_expiry_date[$key] : null;
								$temp_goods_delivery_detail_value['lot_number'] = isset($arr_lot_number[$key]) ? $arr_lot_number[$key] : null;
								$inventory_manage_id = isset($arr_temp_inventory_manage_id[$key]) ? $arr_temp_inventory_manage_id[$key] : 0;
								$warehouse_id = isset($arr_temp_warehouse_id[$key]) ? $arr_temp_warehouse_id[$key] : 0;
								$temp_goods_delivery_detail_value['warehouse_id'] = $warehouse_id;

							//get serial number for MO
								$str_serial_number .= $this->get_serial_number_when_mo_done($goods_delivery_detail_value['commodity_code'], $warehouse_id, $inventory_manage_id, $value);

								$this->warehouse_model->add_goods_transaction_detail($temp_goods_delivery_detail_value, 2);
							}

							if(new_strlen($str_serial_number) > 0){
								$this->db->where('id', $goods_delivery_detail_value['id']);
								$this->db->update(db_prefix().'goods_delivery_detail', ['serial_number' => $str_serial_number]);
							}
						}
    ...
    {
    	$affected_rows=0;
    	$lot_number_from_mo_number = get_mrp_option('lot_number_from_mo_number');
    	$this->load->model('warehouse/warehouse_model');
    	//get product data
		...
		$inventory_purchase_version = mrp_required_inventory_purchase_module('1.3.8');
		if($inventory_purchase_version['inventory']){
			if($lot_number_from_mo_number == 1){
				$good_receipt_detail['lot_number'] = $mo_data->manufacturing_order_code;
			}else{
				$good_receipt_detail['lot_number'] = $this->warehouse_model->create_lot_number(true, true);
			}
		}
    ...
    public function mrp_add_inventory_quantity($data, $status, $warehouse_id)
    {
    	...
    	if ($status == 1) {
			...
			// stock export  with status = 2
			$data_update=[];
    		foreach ($data as $key => $value) {

    			$available_quantity_n =0;
    			$data_warehouse_id ='';
    			$temp_purchase_prices = null;
    			$temp_quantity_with_purchase_price = null;
    			$temp_inventory_manage_id = null;
    			$temp_old_quantities = null;
				$old_quantity =  0;
    			...
    			foreach ($result as $r_key => $r_value) {
    				...
    				$old_quantity  += (float)$r_value['inventory_number'];
    			}
    			...
    			foreach ($result as $result_value) {
    				if (($result_value['inventory_number'] != 0) && ($temp_quantities != 0)) {
						...
						//update inventory
    						...
    						if(new_strlen($temp_quantity_with_purchase_price) != 0){
    							$temp_quantity_with_purchase_price .=','.$result_value['inventory_number'];
    						}else{
    							$temp_quantity_with_purchase_price .= $result_value['inventory_number'];
    						}

    						if(new_strlen($temp_purchase_prices) != 0){
    							$temp_purchase_prices .=','.$result_value['purchase_price'];
    						}else{
    							$temp_purchase_prices .= $result_value['purchase_price'];
    						}

    						if(new_strlen($temp_inventory_manage_id) != 0){
    							$temp_inventory_manage_id .=','.$result_value['id'];
    						}else{
    							$temp_inventory_manage_id .= $result_value['id'];
    						}

    						$old_quantity -= (float)$result_value['inventory_number'];
    						if(new_strlen($temp_old_quantities) != 0){
    							$temp_old_quantities .=','.$old_quantity;
    						}else{
    							$temp_old_quantities .= $old_quantity;
    						}
						...
						//update inventory
    						...
    						if(new_strlen($temp_quantity_with_purchase_price) != 0){
    							$temp_quantity_with_purchase_price .=','.$temp_quantities;
    						}else{
    							$temp_quantity_with_purchase_price .= $temp_quantities;
    						}
    			...
    			array_push($data_update, [
    				...
    				'temp_quantity_with_purchase_price' => $temp_quantity_with_purchase_price,
    				'purchase_prices' => $temp_purchase_prices,
    				'old_quantities' => $temp_old_quantities,
    				'inventory_manage_id' => $temp_inventory_manage_id,
    			]);
	...
	public function mo_add_goods_delivery($mo_data)
	{
		...
		foreach ($mo_detail as $key => $value) {
			...
			if($get_product){
				...
				$arr_item_insert[$key]['mo_detail_id'] = $value['id'];
	...
	public function mo_add_delivery_log($data)
	{
		...
		$this->db->update(db_prefix() . 'goods_delivery_detail', [
			...
			// write log goods transaction detail
	...
	public function get_manufacturing_order_costing($id)
	{
	    ...
	    foreach ($manufacturing_order_details as $key => $value) {
	    	...
	    	$temp_purchase_prices = $value['purchase_prices'];
	    	$temp_quantity_with_purchase_price = $value['temp_quantity_with_purchase_price'];

	    	$arr_temp_purchase_prices = new_explode(',', $temp_purchase_prices);
	    	$arr_temp_quantity_with_purchase_price = new_explode(',', $temp_quantity_with_purchase_price);

	    	foreach ($arr_temp_quantity_with_purchase_price as $key => $value) {
	    		$purchase_price = isset($arr_temp_purchase_prices[$key]) ? $arr_temp_purchase_prices[$key] : 0;
	    		$quantities = $value;

	    		$rate = (float)$purchase_price * (float)$quantities;
		...
		if(count($manufacturing_order) > 0){
			...
			if($get_bill_of_material_details_by_work_order){
				foreach ($get_bill_of_material_details_by_work_order as $work_order) {
				    ...
				    $mo_detail = $this->db->get(db_prefix().'mrp_manufacturing_order_details')->row();

					$qty_to_consume = 0;
					if($mo_detail){
						$qty_to_consume = (float)($mo_detail->qty_to_consume);
				    	$this->db->where('id', $mo_detail->id);
						$this->db->update(db_prefix() . 'mrp_manufacturing_order_details', ['qty_reserved' => $qty_to_consume, 'qty_done' => $qty_to_consume ]);
					}
					...
					$change_type = 'is_use';
					$change_quantity = 0-$qty_to_consume;
					$created_at = date('Y-m-d H:i:s');
					$staff_id = '';
    ...
    /**
     * get serial number when mo done
     * @param  [type] $commodity_id        
     * @param  [type] $warehouse_id        
     ...
     * @return [type]                      
     */
    public function get_serial_number_when_mo_done($commodity_id, $warehouse_id, $inventory_manage_id, $quantity)
    {
    	$str_serial_number = '';
	...
	public function add_approval_setting($data) {
		...
		$this->db->insert(db_prefix() . 'mrp_approval_setting', $data);
		$insert_id = $this->db->insert_id();
		if ($insert_id) {
			return true;
		}
	...
	public function edit_approval_setting($id, $data) {
		...
		$this->db->where('id', $id);
		$this->db->update(db_prefix() . 'mrp_approval_setting', $data);
	...
	public function delete_approval_setting($id) {
		if (is_numeric($id)) {
			$this->db->where('id', $id);
			$this->db->delete(db_prefix() . 'mrp_approval_setting');
	...
	public function get_approval_setting($id = '') {
		if (is_numeric($id)) {
			$this->db->where('id', $id);
			return $this->db->get(db_prefix() . 'mrp_approval_setting')->row();
		}
		return $this->db->get(db_prefix() . 'mrp_approval_setting')->result_array();
	}
	 ...
	 * @return object
	 */
	public function get_approve_setting($type, $status = '1', $json_decode = true) {

		$this->db->select('*');
		$this->db->where('related', $type);
		$approval_setting = $this->db->get('mrp_approval_setting')->row();
		if ($approval_setting) {
			if($json_decode){
	...
	public function get_staff_sign($rel_id, $rel_type) {
		...
		$approve_status = $this->db->get(db_prefix() . 'mrp_approval_details')->result_array();
	 ...
	 * @return  boolean
	 */
	public function check_approval_details($rel_id, $rel_type, $approval_type = 0) {
		$this->db->where('rel_id', $rel_id);
		$this->db->where('rel_type', $rel_type);
		$approve_status = $this->db->get(db_prefix() . 'mrp_approval_details')->result_array();
	...
	public function get_list_approval_details($rel_id, $rel_type) {
		...
		$this->db->where('rel_id', $rel_id);
		$this->db->where('rel_type', $rel_type);
		return $this->db->get(db_prefix() . 'mrp_approval_details')->result_array();
	}
		...
		foreach ($data_new as $value) {
			...
			if ($value->approver !== 'staff') {
				...
				if (is_numeric($approve_value)) {
					...
					$this->db->delete(db_prefix().'mrp_approval_details');
				...
				$row['sender'] = $sender;
				$this->db->insert(db_prefix().'mrp_approval_details', $row);
	...
	public function update_approval_details($id, $data) {
		...
		$this->db->where('id', $id);
		$this->db->update(db_prefix() . 'mrp_approval_details', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		}
		return false;
	...
	/**
	 * get_approve_detail_wh
	 * @param  [type] $data 
	 * @return [type]       
	 */
	public function get_approve_detail_wh($data){
        $this->db->where('rel_id', $data['rel_id']);
        $this->db->where('rel_type', $data['rel_type']);
        $this->db->where('staffid', $data['staff_approve']);
        $details = $this->db->get(db_prefix().'mrp_approval_details')->row();
        if($details){
            return $details;
        }
    ...
    public function send_mail($data ,$staffid = ''){
    	...
    	switch ($data['rel_type']) {
    		case '1':
    		$type = _l('mrp_manufaturing_order');

    		$get_manufacturing_order = $this->get_manufacturing_order($data['rel_id']);
    		if($get_manufacturing_order['manufacturing_order']){
    			$staff_addedfrom = $get_manufacturing_order['manufacturing_order']->addedfrom;
    		}
    		$list_approve_status = $this->get_list_approval_details($data['rel_id'], $data['rel_type']);
    		$mes = 'notify_send_request_approve_manufaturing_order';
    		$mes_approve = 'notify_send_approve_manufaturing_order';
    		$mes_reject = 'notify_send_rejected_manufaturing_order';
    		$link = 'manufacturing/view_manufacturing_order/' . $data['rel_id'];
    		break;
    	...
    	$get_approve_setting = $this->get_approve_setting($data['rel_type'], 1, false);
    	...
    	if(isset($get_approve_setting->approval_type) && $get_approve_setting->approval_type == 0){
    		...
    		$check_approve_status = $this->check_approval_details($data['rel_id'], $data['rel_type'], 1);
    	}

    	if(isset($get_approve_setting->approval_type) && $get_approve_setting->approval_type == 1){
    		...
    		if (isset($check_approve_status['staffid']) && !isset($data['approve'])){
    			...
    			foreach ($check_approve_status['staffid'] as $value) {
    				...
    				if($value != '' && (get_staff_user_id() != $value)){
	...
	public function update_approve_request($rel_id, $rel_type, $status) {
		...
		switch ($rel_type) {
			...
			$data_update['approval'] = $status;
			$this->db->where('id', $rel_id);
			$this->db->update(db_prefix() . 'mrp_manufacturing_orders', $data_update);

			if((int)$status == 1){
				//send notification to Reponsible of Manufacturing Order
				$this->notify_responsible_mo($rel_id);
				//module hook after MO approve
	            hooks()->do_action('after_manufacturing_order_approve', $rel_id);
			}
	...
	/**
	 * notify responsible mo
	 * @param  [type] $mo_id 
	 * @return [type]        
	 */
	public function notify_responsible_mo($mo_id)
	{	
		$manufacturing_order = $this->get_manufacturing_order($mo_id);
		$responsible_name = '';
		$responsible_email = '';
		$responsible_id = 0;
		$mo_number = 0;
		if(isset($manufacturing_order['manufacturing_order'])){
			$staff = get_staff($manufacturing_order['manufacturing_order']->staff_id);
			if($staff){
				$responsible_name = $staff->firstname.' '.$staff->lastname;
				$responsible_email = $staff->email;
			}
			$responsible_id = $manufacturing_order['manufacturing_order']->staff_id;
			$mo_number = $manufacturing_order['manufacturing_order']->manufacturing_order_code;
		}

		// Send notify
		if($responsible_id != 0){
			$mes = 'mrp_you_have_a_manufacturing_order_that_needs_to_be_processed';
			$link = 'manufacturing/view_manufacturing_order/' . $mo_id;

			$notified = add_notification([
				'description' => $mes,
				'touserid' => $responsible_id,
				'link' => $link,
				'additional_data' => serialize([
					'ready_for_mo',
				]),
			...
			if ($notified) {
				pusher_trigger_notification([$responsible_id]);
			}
		}

		// Send mail
		if($responsible_email != ''){
			$content_html = $this->email_content_from_mo_approved('ready_for_mo', $responsible_name, $mo_id, $mo_number);

			$subject = _l('mrp_manufaturing_order').' ['.$mo_number.'] '._l('mrp_needs_to_be_processed');

			$this->load->model('emails_model');
			$result = $this->emails_model->send_simple_email($responsible_email, $subject, $content_html );
			if ($result) {
				return true;
			}
			return false;
	...
	/**
	 * email content from mo approved
	 * @param  [type] $status     
	 * @param  [type] $staff_name 
	 * @param  [type] $mo_id      
	 * @return [type]             
	 */
	public function email_content_from_mo_approved($status, $staff_name, $mo_id, $mo_number)
	{
		$content_html = '';
		$link = 'manufacturing/view_manufacturing_order/' . $mo_id;

		$content_html .= _l('mrp_you_have_a_manufacturing_order_that_needs_to_be_processed').'. '._l('mrp_please_see_details_in_the_link').' <a href="'.admin_url($link).'">'.$mo_number.'</a>';
