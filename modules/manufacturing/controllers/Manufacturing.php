...
class Manufacturing extends AdminController
{
	...
	public function setting()
	{
		...
		if($data['group'] == 'unit_of_measure_categories'){
			...
			$data['approval_setting'] = $this->manufacturing_model->get_approval_setting();
	...
	public function operation_manage($id='')
	{
		...
		if($id != ''){
			...
			if(!isset($data['routing']->id)){
				blank_page('Routing Not Found', 'danger');
			}
		}
	...
	public function bill_of_material_detail_manage($id='')
	{
		...
		if($id != ''){
			...
			if(!isset($data['bill_of_material']->id)){
				show_404();
			}
			if(mrp_get_status_modules('product_lifecycle') == 1){
				$data['routings'] = $this->manufacturing_model->get_routings(false, 'id = '.$data['bill_of_material']->routing_id.' OR status = "active"', false);
			}
		}
	...
	public function add_edit_manufacturing_order($id = '')
	{
			...
			if ($id == '') {
				...
				if ($id) {
					if($data['save_and_send_request'] == 'true'){
						$this->save_and_send_request_send_mail(['rel_id' => $id, 'rel_type' => '1', 'addedfrom' => get_staff_user_id()]);
					}
		...
		if ($id != ''){
			...
			$data['bill_of_materials'] = $this->manufacturing_model->get_bill_of_material_detail_with_product_name(false);
		}
	...
	public function view_manufacturing_order($id)
	{
		...
		$get_approve_setting = $this->manufacturing_model->get_approve_setting('1', '', false);
		if(isset($get_approve_setting->approval_type) && $get_approve_setting->approval_type == 0){
			$data['check_approve_status'] = $this->manufacturing_model->check_approval_details($id, 1);
		}else{
			$data['check_approve_status'] = $this->manufacturing_model->check_approval_details($id, 1, 1);
		}
		$data['list_approve_status'] = $this->manufacturing_model->get_list_approval_details($id, 1);
		$manufacturing_order = $this->manufacturing_model->get_manufacturing_order($id);

		$staff_approval = false;
		$staff_id = get_staff_user_id();

		foreach ($data['list_approve_status'] as $value) {
		...
		if(isset($manufacturing_order['manufacturing_order']) && $manufacturing_order['manufacturing_order']->staff_id == get_staff_user_id()){
			$staff_approval = true;
		}
		
		if (!has_permission('manufacturing', '', 'view')  && !is_admin() && !$staff_approval) {
			access_denied('manufacturing_order');
		}
		...
		$data['get_staff_sign'] = $this->manufacturing_model->get_staff_sign($id, 1);
		
		$check_appr = $this->manufacturing_model->get_approve_setting('1');
	 ...
	 * @return [type]     
	 */
	public function mo_mark_as_done()
	{
		if (!$this->input->is_ajax_request()) {
			show_404();
		}
	...
	public function prefix_number()
	{
		...
		if ($data) {
			if(!isset($data['lot_number_from_mo_number'])){
				$data['lot_number_from_mo_number'] = 0;
			}
	...
	public function approval_setting() {
		if ($this->input->post()) {
			...
			if ($data['approval_setting_id'] == '') {
				$message = '';
				$success = $this->manufacturing_model->add_approval_setting($data);
				if ($success) {
					$message = _l('added_successfully', _l('approval_setting'));
				}
				set_alert('success', $message);
				redirect(admin_url('manufacturing/setting?group=approval_setting'));
			} else {
				$message = '';
				$id = $data['approval_setting_id'];
				$success = $this->manufacturing_model->edit_approval_setting($id, $data);
				if ($success) {
	...
	public function delete_approval_setting($id) {
		...
		$response = $this->manufacturing_model->delete_approval_setting($id);
		if (is_array($response) && isset($response['referenced'])) {
	...
	public function get_html_approval_setting($id = '') {
		...
		$related = [
			['id' => '1', 'name' => _l('mrp_manufaturing_orders')],
		];
		...
		$approver = [
			...
			if (is_numeric($id)) {
				$approval_setting = $this->manufacturing_model->get_approval_setting($id);

				$setting = json_decode($approval_setting->setting);

				$get_approval_setting = $this->manufacturing_model->get_approval_setting();
				$related_exist = [];
				...
				foreach ($setting as $key => $value) {
					...
					if ($key == 0) {
						...
						<div class="col-md-1 button_class mtop25" >
						<span class="pull-bot">
				...
				$approval_setting = $this->manufacturing_model->get_approval_setting();
				$related_exist = [];
	...
	public function send_request_approve() {
		...
		if($data['rel_type'] == '1'){
			...
			$success = $this->manufacturing_model->send_request_approve($data);
		}
	...
	public function approve_request() {
		...
		$get_approve_setting = $this->manufacturing_model->get_approve_setting($data['rel_type'], 1, false);

		if(isset($get_approve_setting->approval_type) && $get_approve_setting->approval_type == 0){
			$check_approve_status = $this->manufacturing_model->check_approval_details($data['rel_id'], $data['rel_type']);
		}else{
			$check_approve_status = $this->manufacturing_model->check_approval_details($data['rel_id'], $data['rel_type'], 1);

		}
		$approval_detail = $this->manufacturing_model->get_approve_detail_wh($data);


		if (isset($data['approve']) && in_array(get_staff_user_id(), $check_approve_status['staffid'])) {

			$success = $this->manufacturing_model->update_approval_details($approval_detail->id, $data);
				...
				if ($data['approve'] == 1) {
					...
					if ($signature != '') {
						...
						// case 'manufacturing order':
							case 1:
							$path = MANUFACTURING_ORDER_UPLOAD_FOLDER . $data['rel_id'];
							break;

							default:
							$path = MANUFACTURING_ORDER_UPLOAD_FOLDER;
							break;
						}
						mo_process_digital_signature_image($signature, $path, 'signature_' . $approval_detail->id);
					...
					if(isset($get_approve_setting->approval_type) && $get_approve_setting->approval_type == 0){
						if ($check_approve_status === true) {
							$this->manufacturing_model->update_approve_request($data['rel_id'], $data['rel_type'], 1);
	...
	public function save_and_send_request_send_mail($data ='') {
		if ((isset($data)) && $data != '') {
			$this->manufacturing_model->send_mail($data);
