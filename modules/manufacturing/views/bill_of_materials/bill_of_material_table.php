...
if(mrp_get_status_modules('product_lifecycle') == 1 && false){
	array_push($where, 'AND ' . db_prefix() . 'mrp_bill_of_materials.status = "active"');
}
...
foreach ($rResult as $aRow) {
	...
	for ($i = 0; $i < count($aColumns); $i++) {
		...
		if($aColumns[$i] == 'id') {
			...
			$_data = get_item_code($aRow['product_id']);

		}elseif ($aColumns[$i] == 'product_id') {
			$code = '<a href="' . admin_url('manufacturing/bill_of_material_detail_manage/' . $aRow['id']) . '">' . get_item_description($aRow['product_id']) . '</a>';
			...
			$_data = '<span class="label label-success">'.$aRow['version'].'</span>';
		}
