...
<table class="table dt-table">
	...
	<?php foreach($approval_setting as $value){ ?>
		<tr>
			...
			$related ='';
			if($value['related'] == 1){
				$related = _l('mrp_manufaturing_orders');
			}
		   ...
		   <td>
            ...
            <?php if (is_admin() || has_permission('manufacturing', '', 'edit') ) { ?>

		     <a href="#" onclick="edit_approval_setting(this,<?php echo new_html_entity_decode($value['id']); ?>); return false" data-name="<?php echo new_html_entity_decode($value['name']); ?>" data-related="<?php echo new_html_entity_decode($value['related']); ?>" data-approval_type="<?php echo new_html_entity_decode($value['approval_type']); ?>" data-setting='<?php echo new_html_entity_decode($value['setting']); ?>' class="btn btn-default btn-icon"><i class="fa-regular fa-pen-to-square"></i></a>
		 	<?php } ?>

            <?php if (is_admin() || has_permission('manufacturing', '', 'delete') ) { ?>

		      <a href="<?php echo admin_url('manufacturing/delete_approval_setting/'.$value['id']); ?>" class="btn btn-danger btn-icon _delete"><i class="fa fa-remove"></i></a>
		  <?php } ?>
...
<div class="modal fade" id="approval_setting_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog approval_model" role="document">
		<div class="modal-content">
			...
			<?php echo form_open('manufacturing/approval_setting',array('id'=>'approval-setting-form')); ?>
			<div id="approval_id">
			...
			<div class="modal-body">
				<div class="row">
					<div class="col-md-12">
						...
						<?php $related = [ 
								0 => ['id' => '1', 'name' => _l('mrp_manufaturing_orders')],
							]; ?>
						...
						<div class="form-group">
							<div class="checkbox checkbox-primary">
								<input type="checkbox" id="approval_type" name="approval_type" value="1">
								<label for="approval_type"><?php echo _l('wh_approval_type_label'); ?>
								<a href="#" class="pull-right display-block input_method"><i class="fa fa-question-circle i_tooltip" data-toggle="tooltip" title="" data-original-title="<?php echo _l('wh_approval_type_tooltip'); ?>"></i></a>
							</label>
						</div>
					</div>
