...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, ['id', 'approval']);
...
foreach ($rResult as $aRow) {
	...
	for ($i = 0; $i < count($aColumns); $i++) {
		...
		if($aColumns[$i] == 'id') {
			...
			if($aRow['approval'] == 1){
				$code .= '<span class="label label-tag tag-id-1 label-tab1 mleft5"><span class="tag">'._l('approved').'</span><span class="hide">, </span></span>&nbsp';
			}elseif($aRow['approval'] == 0){
				$code .= '<span class="label label-tag tag-id-1 label-tab2 mleft5"><span class="tag">'._l('not_yet_approve').'</span><span class="hide">, </span></span>&nbsp';
			}elseif($aRow['approval'] == -1){
				$code .= '<span class="label label-tag tag-id-1 label-tab3 mleft5"><span class="tag">'._l('reject').'</span><span class="hide">, </span></span>&nbsp';
			}
			...
			$_data = ' <span class="label label-'.$aRow['status'].'" > '._l('mrp_'.$aRow['status']).' </span>';
