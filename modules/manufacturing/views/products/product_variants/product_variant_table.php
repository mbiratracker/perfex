...
$where[] = 'AND '.db_prefix().'items.active = 1 AND '.db_prefix().'items.parent_id is not null AND  '.db_prefix().'items.parent_id != 0 AND  '.db_prefix().'items.attributes is not null ';
...
if(mrp_get_status_modules('product_lifecycle') == 1){
	$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix().'items.id', 'next_version']);
}else{
	...
	for ($i = 0; $i < count($aColumns); $i++) {
		...
		if($aColumns[$i] == db_prefix() . 'items.id'){
			if (count($arr_images) > 0) {
				...
				}elseif (file_exists('item_img/modules/warehouse/uploads/' . $arr_images[0]['rel_id'] . '/' . $arr_images[0]['file_name'])) {
		...
		if ($aColumns[$i] == 'description') {
			...
			$_version = '<span class="label label-success">'.$aRow['version'].'</span>';
			if(!is_null($aRow['next_version']) && strlen($aRow['next_version']) > 0){
				$_version .= ' -> <span class="label label-warning">'.$aRow['next_version'].'</span>';
			}
			$_data = $_version;
		}elseif($aColumns[$i] == '1'){
