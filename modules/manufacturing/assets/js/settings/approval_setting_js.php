    ...
    function new_approval_setting(){
      ...
      $('#approval_id').html('');
      $('#approval_id').append(hidden_input('approval_setting_id',''));

      $('select[name="related"]').val('').change();
      $('input[name="name"]').val('');

      $('#approval-setting-form input[id="approval_type"]').prop('checked', false);

      $.post(admin_url + 'manufacturing/get_html_approval_setting').done(function(response) {
       response = JSON.parse(response);

       $('.list_approve').html('');
       $('.list_approve').append(response.html);

       $('select[name="related"]').html('');
       $('select[name="related"]').append(response.related_html);
    ...
    function edit_approval_setting(invoker,id){
      ...
      if(approval_type == 1){
        $('#approval-setting-form input[id="approval_type"]').prop('checked', true);
      }else{
        $('#approval-setting-form input[id="approval_type"]').prop('checked', false);
      }

      $.post(admin_url + 'manufacturing/get_html_approval_setting/'+ id).done(function(response) {
       response = JSON.parse(response);

       $('.list_approve').html('');
       $('.list_approve').append(response.html);
       addMoreVendorsInputKey = response.index;
